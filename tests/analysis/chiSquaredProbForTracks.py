...
db = MySQLdb.connect(host=mySqlHost, user=mySqlUser, passwd=mySqlPasswd,
                     db=mySqlDb, unix_socket='/tmp/mysql.sock')
ephemDb = MySQLdb.connect(host=mySqlHost, user=mySqlUser, passwd=mySqlPasswd,
                          db=mySqlEphemDb, unix_socket='/tmp/mysql.sock')
dtMin = 0.01
...
def getSsmTrack(ssmId):
    ...
    c = ephemDb.cursor()
    query = 'SELECT diaSourceID, taiMidPoint, ra, decl from dias_nodeep where ssmID=%s order by taiMidPoint' % (
        ssmId)
    c.execute(query)
    result = c.fetchall()
    for row in result:
